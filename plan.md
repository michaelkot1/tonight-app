...
- **Share:** Keep RN `Share` (no new clipboard package in Wave 1). Clipboard polish skipped in Wave 2 (timeboxed).

**Wave 2 orchestrator locks (2026-09-10):**
- Contacts UI: Friends primary + optional CTA on onboarding invite.
- Match action: suggest + explicit Follow (not auto-follow); email-only matching.
- Social lines: Loved-first; max ~3 avatars; copy like “Alex & 2 friends loved this”.

**Status (2026-09-10):** 🟢 **Wave 1 + Wave 2 implemented** on `cursor/phase-4-friends-invites` (uncommitted; pending owner smoke-test).
- Wave 1: `create_or_get_my_invite()` + invite invite flush + `/invite/[code]` + Friends (@search / follow) + onboarding invite share.
- Wave 2: `match-contacts` Edge Function (`verify_jwt`) + service-role RPC `match_profiles_by_emails`; `expo-contacts` + Friends/onboarding match UI (suggest Follow); `FriendPile` + `useFriendSocialByTitle` wired on Home hero/rails + title detail.
- Verify: `tsc`, `expo lint`, `expo export --platform ios` clean.
- **Owner smoke-test:** invite + @handle follow; contacts permission → matches → Follow; Home/detail piles when followed friends rated overlapping titles.
